...
def _sub(msg: str):   _log.info("  └─ %s", msg)
...
def georef_scanners(scanner_cfg_paths: list[Union[str, Path]], pipe_cfg: dict) -> list[dict]:
    ...
    _info(f"[georef] {scenario_name} — {len(scanner_cfg_paths)} scanner(s)")
    ...
    for scanner_cfg_path in scanner_cfg_paths:
        ...
        _sub(f"{scanner_name} → {output_dir}")
        run_from_yaml(str(generated_cfg_path))
        _info(f"[georef] {scanner_name} ✓")
    ...
    _info(f"[georef] done — {len(scanners_meta)} scanner(s) processed")
...
def build_merged_time_manifest(
    ...
    _info(f"[manifest] written: {manifest_path} ({len(rows)} rows)")
...
def load_vux_infos_from_manifest(vux_dir: Path, manifest_path: Path):
    ...
    vux_infos = []
    _info("[VUX+PUCK] VUX temporal bounds (manifest)")
    _info("======================================")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Analyse temps VUX", unit="file"):
        ...
        _info(f"{f.name} | {info['t_min']:.6f} -> {info['t_max']:.6f}")
...
def merge_vux_group_with_puck_by_time(
    ...
    _info("[VUX+PUCK] PUCK temporal bounds")
    _info("======================================")
    for f in tqdm(puck_files, desc="Analyse temps PUCK", unit="file"):
        ...
        _info(f"{f.name} | n={n} | {t_min:.6f} -> {t_max:.6f}")

    _info("[VUX+PUCK] Merge VUX group + PUCK")
    _info("======================================")
    ...
    for i, vux_info in enumerate(tqdm(vux_infos, desc="Merge VUX->ALL", unit="cloud")):
        ...
        _info(f"[{i+1}/{len(vux_infos)}] VUX file: {vux_file.name}")
        _info(f"Window: ({prev_end:.6f}, {'+inf' if is_last else f'{current_end:.6f}'}]")
        _info(f"Output: {out_file}")
        _info("--------------------------------------")
                ...
                for puck_file in tqdm(puck_files, desc=f"PUCK for {vux_file.stem}", unit="file", leave=False):
                    with laspy.open(puck_file) as puck_reader:
                        ...
                        if selected_this_file > 0:
                            _info(f"[PUCK] {puck_file.name}: {selected_this_file} selected point(s)")
    ...
    _info(f"[VUX+PUCK] Manifest written: {manifest_out}")
    _info("[VUX+PUCK] scanner_src: 2=VUX, 1=PUCK")
...
def merge_scanner_group(
    ...
    _info(f"[merge] Starting merge group: {group_name}")
    _info(f"[merge] scanners: {' + '.join(scanner_names)}")
    for e in group_entries:
        _info(f"[merge] {e['scanner_name']} dir: {e['output_dir']}")
    _info(f"[merge] OUT dir: {out_dir}")

    if len(group_entries) == 1:
        _info(f"[merge] Group {group_name} has only one scanner -> no merge needed.")
        ...
        for i in range(1, len(group_entries)):
            ...
            _info("--------------------------------------")
            _info(f"[merge] step {i}/{len(group_entries)-1}")
            _info(f"[merge] A: {current_dir}")
            _info(f"[merge] B: {next_dir}")
            _info(f"[merge] target: {target_dir}")
            _info("--------------------------------------")
    ...
    _info(f"[merge] Merge group completed: {group_name}")
    _info(f"[merge] final dir: {current_dir}")

    
    detected_fmt = detect_cloud_format_in_dir(current_dir)

    _sub(f"detected cloud format: {detected_fmt}")
...
def georef_and_merge(pipe_cfg: dict) -> dict:
    ...
    if pipe_cfg["steps"].get("merge", False):
        ...
        _info(f"[merge] preset={preset} | scanners: {list(entry_map.keys())}")
        ...
        def _merge_puck_on_vux(vux_dir: Path):
            ...
            _info("[merge] Adding PUCK to VUX merged clouds")
            _info(f"[merge] vux_dir:      {vux_dir}")
            _info(f"[merge] puck_dir:     {puck_dir}")
            _info(f"[merge] manifest:     {manifest_path}")
            _info(f"[merge] out_dir:      {final_out_dir}")
        ...
        def _cleanup_dir(path: Path, label: str) -> None:
            ...
            path = Path(path)
            if not path.exists():
                _info(f"[merge/cleanup] {label}: already absent, skipping ({path})")
                return
            _info(f"[merge/cleanup] Removing {label}: {path}")
            shutil.rmtree(path)
            _info(f"[merge/cleanup] Removed: {path}")
        ...
        if preset == "vux_only":
            ...
            if cleanup_enabled:
                _info("[merge/cleanup] preset=all — removing intermediate dirs")
                _info("======================================")
...
def combined_multi_outage_scenario(
    ...
    _info("[combined scenario]")
    _info(f"Outages: {outages}")
    _info(f"Expanded intervals: {intervals}")
    _info(f"cloud_fmt: {cloud_fmt}")
    if cloud_fmt in {"las", "laz"}:
        _info(f"time_field: {time_field}")
    ...
    if not do_chunks:
        _info(f"[combined] reuse existing chunks -> {chunks_root}")
    ...
    if use_manifest and manifest_path.exists():
        _info(f"[combined] Using manifest: {manifest_path}")
        ...
        if use_manifest:
            _info(f"[combined] Manifest not found: {manifest_path}")
        _info("[combined] Fallback to direct cloud scan")
    ...
    try:
        ...
        _info(f"[combined] Trajectory loaded ({len(_t_trj)} poses)")

    except Exception as e:
        _info(f"[combined] WARNING: no trajectory ({e})")
    ...
    if _t_trj is not None:
        ...
        for las_path in las_files:
            try:
                ...
                _info(f"[bbox] Warning: {las_path.name}: {e}")
    ...
    _info(f"[combined] chunks root: {chunks_root}")
...
def run_limatch_api(
    ...
    if cfg_overrides:
        ...
        _info(f"[limatch-api] overrides: {cfg_overrides}")
    ...
    _info(f"[limatch-api] prj_folder={cfg['prj_folder']}")
...
def run_limatch_on_chunks_per_scan(
    ...
    _info("[limatch] Running LiMatch on chunks")
    _info(f"[limatch] chunks_root: {chunks_root}")
    _info(f"[limatch] out_root:    {out_root}")
    _info(f"[limatch] cloud_fmt:   {cloud_fmt}")
    _info(f"[limatch] pattern:     {chunk_pattern}")
    _info(f"[limatch] neighbor_k = {neighbor_k}")
    _info(f"[limatch] intra-scan: chunk_i ↔ chunk_(i+1 ... i+{neighbor_k})")
    _info(f"[limatch] cross-scan: {do_cross_scan}")
    _info(f"[limatch] total scans: {len(scan_dirs)}")
    ...
    for scan_dir in scan_dirs:
        ...
        if len(chunk_files) < 1:
            _info(f"[limatch] skip {scan_dir.name}: no chunks matching {chunk_pattern}")
            continue
        ...
        if not pairs:
            _info(f"[limatch] skip {scan_dir.name}: not enough pairs")
            continue
...
def find_spatial_crossing_pairs(
    ...
    for scan_dir in scan_dirs:
        ...
        if not bbox_path.exists():
            _info(f"[crossings] No chunk_bbox.csv in {scan_dir.name} — skip")
            continue
    ...
    if not all_chunks:
        _info("[crossings] No bbox index found — cannot detect crossings")
        return []
    ...
    n = len(rows)
    _info(f"[crossings] {n} chunks — min_sep={min_chunk_separation}m  margin={overlap_margin_m}m")
    ...
    _info(f"[crossings] Found {len(pairs)} crossing pairs")
    return pairs
...
def run_limatch_on_spatial_crossings(
    ...
    if not crossing_pairs:
        _info("[crossings] No crossing pairs found — nothing to do")
        return
    ...
    for a, b, merged_parent in crossing_pairs:
        ...
        _info(f"  {merged_parent}/{pair_name}")
        try:
            run_limatch_api(
                repo_root=repo_root,
                limatch_cfg_path=limatch_cfg,
            ...
            _info(f"  [FAIL] {pair_name}: {type(e).__name__}: {e}")
            continue

    _info("[crossings] Done")
...
def run_limatch_on_patcher_outputs(
    ...
    _info("[limatch] Running LiMatch on Patcher outputs")
    _info(f"[limatch] patcher_out_root: {patcher_out_root}")
    _info(f"[limatch] out_root:         {out_root}")
    _info(f"[limatch] total pairs:      {len(pair_dirs)}")

    for pair_dir in pair_dirs:
        clouds = sorted(pair_dir.glob(file_glob))

        if len(clouds) < 2:
            _info(f"[limatch] SKIP {pair_dir.name}: found {len(clouds)} file(s) (need 2).")
            continue

        try:
            cloud1, cloud2 = _pick_pair_files_any(pair_dir, file_glob=file_glob)
        except Exception as e:
            _info(f"[limatch] SKIP {pair_dir}: {e}")
            continue
        ...
        _info(f"[limatch] pair={pair_name} | cloud1={cloud1.name} | cloud2={cloud2.name}")
        ...
        try:
            ...
            _info(f"[limatch] FAIL {pair_name}: {type(e).__name__}: {e}")
            continue
...
def merge_limatch_correspondences(
    ...
    _info("[merge_correspondences] Merging LiMatch correspondences")
    _info(f"[merge_correspondences] workflow_type: {workflow_type}")
    _info(f"[merge_correspondences] limatch_root:  {limatch_root}")
    _info(f"[merge_correspondences] pattern:      {pattern}")
    _info(f"[merge_correspondences] n files:      {len(files_to_merge)}")
    _info(f"[merge_correspondences] output_file:  {output_file}")
    ...
    _info(f"[merge_correspondences] Done: {output_file}")
    return output_file
# ============================================================================
# 4) Patcher as CLI 
# ============================================================================
...
def run_pipeline(pipe_cfg: dict) -> None:
    ...
    _info(f"[pipeline] {scenario_name} | mode={mode} | out={root_out_dir}")
    _sub(f"limatch_cfg: {limatch_cfg}")
    ...
    _info(f"[pipeline] merged input → {merged_out}")
    _sub(f"georef ref cfg: {cfg_georef_path}")
    ...
    if mode == "chunk":
        ...
        if chunk_source == "existing":
            ...
            _info(f"[chunk] existing → {chunks_root}")
            ...
            if chunk_variant_type == "standard":
                ...
                _info(f"[chunk] outage-window → {chunks_root}")
                _sub(f"GPS outage file: {gps_outage}")
    ...
    # ------------------------------------------------------------
        ...
        if steps_cfg.get("gps_outage_file", False) and gps_outage_block.get("enabled", False):
            ...
            _info(f"[patcher] GPS outage generated: {gps_out}")
            _info(f"[patcher] kept={kept}, removed={removed}")
...
def log_pipeline_config(cfg: dict):
    ...
    steps = cfg.get("steps", {})
    paths = cfg.get("paths", {})
    ow    = cfg.get("chunk_variant", {}).get("outage_window", {})
    active_steps = [k for k, v in steps.items() if v]

    _info("─" * 60)
    _info(f"[pipeline] scenario : {cfg.get('scenario_name')}  |  mode : {cfg.get('mode')}")
    _info(f"[pipeline] steps    : {' → '.join(active_steps) if active_steps else '—'}")
    _sub(f"traj      : {traj.get('path', '—')}")
    _sub(f"out       : {paths.get('root_out_dir', '—')}")
    scanners = cfg.get("scanners", {})
    for key, path in scanners.items():
        try:
            name = get_scanner_name(path)
        except Exception:
            name = key
        _sub(f"scanner   : {key} → {name}")
    if ow.get("outages"):
        for t0, dur in ow["outages"]:
            _sub(f"outage    : {t0} + {dur}s  (pre={ow.get('pre_s',0)}s  post={ow.get('post_s',0)}s)")
    _info("─" * 60)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LiDAR processing pipeline")
